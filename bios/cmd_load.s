...
@read_header_option:
    ...
    tax
@loop_header_option:
    jsr xmodem_get_byte       ; ignore data
    dex
...
@item_found:
    ...
    jsr xmodem_get_byte ; read src addr LSB
    sta (cur_rel)        ; save it
    jsr xmodem_get_byte ; read src addr MSB
    ldy #1
...
@loop:
    ...
    jsr xmodem_get_byte ; swallow segid
    jsr xmodem_get_byte ; swallow addr LSB
    jsr xmodem_get_byte ; swallow addr LSB
...
process_relocation:
    jsr xmodem_get_byte ; get offset byte
...
.code
    jsr xmodem_get_byte    ; read typebyte|segID
...
segid_undefined:
    ...
    jsr xmodem_get_byte ; read index LSB
    asl ; *2
    php ; save carry bit
    clc
    ...
    inc cur_dst_import+1
@skip_msb:
    jsr xmodem_get_byte ; read index MSB
...
segid_absolute:
    ...
    jsr xmodem_get_byte ; swallow low_byte
@end:
    rts
...
@type_high:
    ...
    jsr xmodem_get_byte  ; no, read len_byte (hot path)
    sta len
    txa                  ; get offset LSB
